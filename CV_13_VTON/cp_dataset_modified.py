...
class CPDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.stage == 'GMM':
            ...
            cm = Image.open(osp.join(self.data_path, 'cloth-mask', c_name.split('.')[0] + '.png')).convert('L')
        else:
        ...
        # load parsing image
        parse_name_label = im_name.replace('0.jpg', '4.png')
        parse_name_image_mask = im_name.replace('.jpg', '.png')

        im_parse = Image.open(
            # osp.join(self.data_path, 'image-parse', parse_name)).convert('L')
            osp.join(self.data_path, 'label_maps', parse_name_label)).convert('L')  # updated new segmentation

        im_parse = cv2.imread(osp.join(self.data_path, 'label_maps', parse_name_label), cv2.IMREAD_GRAYSCALE)
        parse_array = np.array(im_parse)


        im_mask = Image.open(
            osp.join(self.data_path, 'image-mask', parse_name_image_mask)).convert('L')
        ...
        if self.stage == 'GMM':
            ...
            parse_head = (parse_array == DressCode_labelmap_g[1]).astype(np.float32) + \
                ...
                (parse_array == DressCode_labelmap_g[4]).astype(np.float32) + \
                (parse_array == DressCode_labelmap_g[9]).astype(np.float32) + \
                (parse_array == DressCode_labelmap_g[12]).astype(np.float32) + \
                (parse_array == DressCode_labelmap_g[13]).astype(np.float32) + \
                (parse_array == DressCode_labelmap_g[16]).astype(np.float32) + \
                (parse_array == DressCode_labelmap_g[17]).astype(
                np.float32)  # CP-VTON+ TOM input (reserved regions)

        parse_cloth = (parse_array == DressCode_labelmap_g[9]).astype(np.float32) + \
            (parse_array == DressCode_labelmap_g[22]).astype(np.float32)     # upper-clothes labels
        ...
        with open(osp.join(self.data_path, 'keypoints-openpose', pose_name), 'r') as f:
            pose_label = json.load(f)
            pose_data = pose_label['people']['pose_keypoints']
        ...
        if self.stage == 'GMM':
            im_g = cv2.imread('grid.png')
            im_g = cv2.cvtColor(im_g, cv2.COLOR_BGR2RGB)
            im_g = cv2.resize(im_g, (self.fine_width, self.fine_height), interpolation=cv2.INTER_NEAREST)
