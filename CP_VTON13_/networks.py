...
from MultiTrans import MULTModel
...
class UnetGenerator(nn.Module):
    def __init__(self, input_nc, output_nc, num_downs, ngf=64,
        ...
        self.multiTrans = MULTModel() #CIT
        self.gamma = nn.Parameter(torch.zeros(1))
        self.attconv = nn.Conv2d(26, 3, 1)

    def forward(self, input):
        agnostic_fea = input[:, 0:22, :, :]
        c_fea = input[:, 22:25, :, :]
        cm_fea = input[:, 25:26, :, :]

        multi_feature = self.multiTrans(agnostic_fea, c_fea, cm_fea).cuda()
        input = input + self.gamma * multi_feature
        transAttFea = self.attconv(multi_feature)

        return self.model(input), transAttFea
...
class GMM(nn.Module):
    ...
    def __init__(self, opt):
        ...
        self.regression = FeatureRegression(
            input_nc=768, output_dim=2*opt.grid_size**2, use_cuda=True)
        self.gridGen = TpsGridGen(
            opt.fine_height, opt.fine_width, use_cuda=True, grid_size=opt.grid_size)

    def forward(self, inputA, inputB):  # agnostic, cm
